...
class Fourier_Model(nn.Module):
    def __init__(self, shape_in, hid_S=512, hid_T=256, N_S=4, N_T=8, incep_ker=[3,5,7,11], groups=8):
        ...
        _, _, self.H, self.W = shape_in
        self.fourier = GF_Block(img_size=(self.H, self.W),
                                patch_size=16,
                                in_channels=2,
                                out_channels=2,
                                input_frames=12,
...
if __name__ == '__main__':
    x = torch.randn((1, 12, 2, 64, 128))
    y = torch.randn((1, 12, 2, 64, 128))
    model1 = Fourier_Model(shape_in=(12,2,64,128))
    output = model1(x)
