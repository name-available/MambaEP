...
class MixedTemperatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        output_img = self.data["y"][index].reshape(12, 2, 32, 64)
        if self.transform:
...
class WindDataset(Dataset):
    ...
    def __getitem__(self, index):
        input_img = self.data["x"][index].reshape(12, 2, 32, 64) 
        output_img = self.data["y"][index].reshape(12, 2, 32, 64)
        if self.transform:
            img = self.transform(img)
...
def load_data(mixed = False, root = 'dataset/component_of_wind/component_of_wind', batch_size = 8):
    ...
    if mixed:
        ...
        train_set = WindDataset(train_set)
        test_set = WindDataset(test_set)
        val_set = WindDataset(val_set)
