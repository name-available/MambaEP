...
from module_MamSB import *
from module_Temporal_block import *
from module_basic import *


class MambaEarthPred_model(nn.Module):
    def __init__(self, shape_in, hid_S=256, hid_T=128, N_S=4, N_T=8, incep_ker=[3,5,7,11], groups=8):
        super(MambaEarthPred_model, self).__init__()
        T, C, H, W = shape_in
        self.H1 = int(H / 2 ** (N_S / 2)) + 1 if H % 3 == 0 else int(H / 2 ** (N_S / 2))
        self.W1 = int(W / 2 ** (N_S / 2))

        self.mamsb_encoder = MamSB(shape_in=shape_in)
    ...
    def forward(self, input_st_tensors):
        ...
        spatial_feature = self.mamsb_encoder(input_st_tensors)
...
if __name__ == '__main__':
    x = torch.randn((1, 10, 9, 64, 64))
    y = torch.randn((1, 10, 1, 64, 64))
    model1 = MambaEarthPred_model(shape_in=(10, 9, 64, 64))
    output = model1(x)
